...
if (x<-2){
	sol=real(exp(I*k*x)+BCDEFGHI(0)*exp(-I*k*x));
	return sol;

	}
	else if (x>-2 && x<-1){
	sol=real(BCDEFGHI(1)*exp(I*nk*x)+BCDEFGHI(2)*exp(-I*nk*x));
	return sol;
	
	}
	
	else if (x>-1 && x<1){
	sol=real(BCDEFGHI(3)*exp(I*nk_2*x)+BCDEFGHI(4)*exp(-I*nk_2*x));
	return sol;
	
	}
	
	else if (x>1 && x<2){
	sol=real(BCDEFGHI(5)*exp(I*nk*x)+BCDEFGHI(6)*exp(-I*nk*x));
	return sol;
	
	}
	
	else {
	sol=real(BCDEFGHI(7)*exp(I*k*x));
	return sol;
