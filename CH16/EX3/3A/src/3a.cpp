...
int main(int argc, char ** argv ) {
  ...
  // MAKE 4 PLOT VIEW (DISPLAY 2 IN "SILENT MODE);
  PlotView view1, view2, view3, view4;
  if (!silent) window.add(&view1, "Fits");
  if (!silent) window.add(&view2, "Measurements");
  window.add(&view3, "Quality");
  window.add(&view4, "Accuracy");

  // DEFINE PLOT LABELS:
  std::map<const PlotView *, std::string>
    xLabelMap={{&view1, "x"}, {&view2, "α"}, {&view3, "Probability χ²"}, {&view4, "Probability χ²"}},
    yLabelMap={{&view1,  "ρ(x)"}, {&view2, "β"}, {&view3, "experiments/bin"}, {&view4, "experiments/bin"}},
    titleMap={{&view1,  "Probability χ²="}, {&view2, "Probability χ²="}, {&view3, "fit quality"}, {&view4, "fit accuracy"}};
 ...
 // for (unsigned int i=0; i<NRUNS; i++) {
   ...
   hist.accumulate(x);};
	...
	view1.setRect(rect);
      ...
      // ADD FIT QUALITY VALUE TO THE TITLE OF PLOT 1
      {
  ...
  // SILENT OR NOT:  END WITH FIT QUALITY PLOT
  PlotHist1D pHistQ(fitQuality);
  view3.add(&pHistQ);
  {
    PRectF rect=pHistQ.rectHint();
  ...
  prop.pen.setWidth(1);
  pHistQ.setProperties(prop);
  pHistA.setProperties(prop);
  

  // ADD A LABEL KEY TO PLOT 3
  PlotKey key3(0.2, 0.75*pHistQ.rectHint().bottom()+0.25*pHistQ.rectHint().top());
  key3.setFont(font);
  key3.add(&pHistQ, "fit quality");
  view3.add(&key3);

  // ADD A LABEL KEY TO PLOT 4
  PlotKey key4(0.2, 0.75*pHistA.rectHint().bottom()+0.25*pHistA.rectHint().top());
  key4.setFont(font);
  key4.add(&pHistA, "fit accuracy");
  view4.add(&key4);
